...
This section presents a performance study of our \SPSER transactional memory depicted in \refsection{stm}.
...
We run \refalg{stm} in disjoint-access parallel (DAP) mode.
As explained in \refsection{stm:discussion}, in this variation the clocks of the processes are not accessed.
A detailed evaluation of the other variations of \refalg{stm} is subject to future work.
...
\reffigure{bank} presents our result for the bank benchmark.
...
\reffigure{bank:speedup} presents the speedup obtained when varying locality from 0, i.e., all the accounts are chosen at random, to 1, they are all chosen in the local branch.
...
As illustrated with \reffigure{bank:speedup}, our TM implementation leverages the presence of data locality.
This is expected as we use the disjoint-access parallel (DAP) variation of \refalg{stm}.
...
\begin{figure}[!t]
  \centering
  \subfigure[Base scenario -- $2^{10}$ accounts, locality set to $0.8$]{
...
The linked-list benchmark consists in modifying a sorted linked list of integers concurrently. 
...
In \reffigure{llrb}, we observe that TinySTM outperforms our implementation in the linked-list benchmark.
...
To alleviate this issue, we plan in a future work to adjust dynamically the clocks used in \refalg{stm} accordingly to contention.
...
Leveraging this workload property, our implementation of \refalg{stm}, scales this application linearly with the number of threads.
