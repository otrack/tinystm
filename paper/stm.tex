...
\refalg{stm} depicts the pseudo-code of our STM construction.
...
Transactions that execute \refalg{stm} manipulate registers.
...
\end{inparaenum}
\refalg{stm} updates incrementally $\readSetOf{T}$ and $\writeSetOf{T}$ on the course of the execution, checking with the help of $\clockOf{T}$ that the transaction is consistent.
With more details, the execution of transaction $T$ proceeds as follows.

\begin{itemize}
\item[-] %
  When $T$ starts its execution, \refalg{stm} initializes
  $\clockOf{T}$ to the value of $\clockOf{p}$, then both $\readSetOf{T}$ and $\writeSetOf{T}$ to $\emptySet$ (\reflines{stm:start:1}{stm:start:3}).
\item[-] %
  ...
  \item in case $x$ was previously accessed $x$, that $T$ retrieves the same version.
  \end{inparaenum}
...
\item[-] %
  ...
  The timestamp of $x$ at the time \refalg{stm} takes the lock serves two purposes.
  First, \refalg{stm} checks that it is lower than the current clock of $T$, and if not $T$ is extended (\refline{stm:write:4}).
  Second, it is saved in \writeSetOf{T} to ensure that at commit time the timestamp of the version of $x$ written by $T$ is greater.
\item[-] %
...
In this section, we prove the three guarantees \refalg{stm} provides.
We first show that our STM design is weakly progressive and strictly serializable.
Then, when $\clockOf{p}$ is local, we also prove that \refalg{stm} is strictly disjoint-parallel.
...
\begin{inparaenum}[\em(i)]
\item \refalg{stm} has to access the read set when the transaction re-reads a location, and
\item when a transaction $T$ reads a location written by a transaction $T'$  
  unknown from the process that executes $T$, \refalg{stm} needs to call \stmExtend{}.
  % Avni:2008 points out this last problem
\end{inparaenum}
