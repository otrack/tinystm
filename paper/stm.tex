...
\refalg{stm} executes it only when a shared object appears to have been recently updated, or when transaction terminates.
...
Consider some run $\run$ of \refalg{stm}, and let $h$ be the history induced by \run.
...
\begin{proof}
  ...
  \item[(3)]
    ...
    At the time, $T_i$ executes this write, the transaction must own a lock on object $x$ (\refline{stm:commit:4}).
    The object is then unlocked by transaction $T_i$ (\refline{stm:commit:5}).
  ...
  \item[(4)]
    ...
    It follows that $T_i $ reads object $x$ at \refline{stm:extend:5} after the write of version $x_j$ occurs.
